...
public class PrintBillingCommand implements BillingCommand {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) 
        ...
        try {
            ...
            request.setAttribute("pageTitle", "Print Billing - " + billing.getBillNumber());
            request.getRequestDispatcher("views/billing-print.jsp").forward(request, response);
            ...
            e.printStackTrace();
            HttpSession session = request.getSession();
            session.setAttribute("errorMessage", "Error retrieving billing for print: " + e.getMessage());
            response.sendRedirect(request.getContextPath() + "/BillingServlet?action=billings");
        }
    }
}