...
/**
 * Factory class to create command objects based on action Updated to support
 * billing operations
 */
public class CommandFactory {
	...
	// Client Actions
	private static final String[] CLIENT_ACTIONS = { "clients", "add", "edit", "delete", "search" };

	// Billing Actions
	private static final String[] BILLING_ACTIONS = { "billings", "create", "view", "print", "complete", "cancel",
			"delete" };
	...
	public static ClientCommand createClientCommand(String action, ClientService clientService) {
		...
		case "edit":
			return new EditClientCommand(clientService);
		case "delete":
	...
	public static BillingCommand createBillingCommand(String action, BillingService billingService,
		...
		switch (action.toLowerCase()) {
		case "billings":
			return new ViewBillingCommand(billingService, clientService, bookService); // Pass clientService and
																						// bookService
		case "create":
